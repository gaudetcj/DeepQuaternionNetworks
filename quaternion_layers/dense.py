...
class QuaternionDense(Layer):
    ...
    def call(self, inputs):
        input_shape = K.shape(inputs)
        input_dim = input_shape[-1] // 4
        r_input = inputs[:, :input_dim]
        i_input = inputs[:, input_dim:input_dim*2]
        j_input = inputs[:, input_dim*2:input_dim*3]
        k_input = inputs[:, input_dim*3:]
        
        cat_kernels_4_r = K.concatenate(
            [self.r_kernel, -self.i_kernel, -self.j_kernel, -self.k_kernel],
            axis=-1
        )
        cat_kernels_4_i = K.concatenate(
            [self.i_kernel, self.r_kernel, -self.k_kernel, self.j_kernel],
            axis=-1
        )
        cat_kernels_4_j = K.concatenate(
            [self.j_kernel, self.k_kernel, self.r_kernel, -self.i_kernel],
            axis=-1
        )
        cat_kernels_4_k = K.concatenate(
            [self.k_kernel, -self.j_kernel, self.i_kernel, self.r_kernel],
            axis=-1
        )
